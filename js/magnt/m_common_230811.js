(function($, window) {
  ...
  var etc = {
    ...
    bind: function() {
      ...
      // 모바일 키패드 열릴 때 하단 버튼 고정 처리
      common.el.doc
        .find('input[type="text"],input[type="tel"],input[type="email"], input[type="number"], input[type="password"], textarea')
        .each(function() {
          var $this = $(this);
          ...
          jessBtn.on('touchend.etc', function(e) {
            ...
            if (eTime - sTime < 700) {
              if (start < end && $this.hasClass('dragTop')) {
                ...
                $(common.selector.bh).css({'overflow': 'visible'});
                $(common.selector.bh).off('scroll touchmove mousewheel');
              } else if (start > end && !$this.hasClass('dragTop')) {
                //up
                $this.addClass('dragTop');
                $(common.selector.bh).css({'overflow': 'hidden'});
                $(common.selector.bh).on('scroll touchmove mousewheel', function(e) {
                  e.preventDefault();
                  e.stopPropagation();
                })
              }
      ...
      $('#pop_searchSetting').each(function() {
        ...
        btn.on('click', function(e){
          ...
          .stop()
          .animate({
            // scrollTop: location[$('#'+tabControl).index()-1] - fixedHeight
            scrollTop: move
          }, 300);
        })
      })
