  ...
  var utils = {
    ...
    dimmedShow: function() {
      ...
      if (dimmed.is(':visible')) {
        ...
        dimmed.show().stop().animate({ opacity: 1 }, 300);
      }
    },
    //딤 hide
    dimmedHide: function() {
      ...
      if (dimmed.hasClass('depth2')) {
        ...
        dimmed.stop().animate({ opacity: 0 }, 200, function() {
          $(this).hide();
        });
      }
    }
  ...
  var header = {
    ...
    handler: {
      ...
      scroll: function() {
        ...
        if (floatingBtns && floatingBtns.classList.contains('floatAbs')) {
          floatingBtns.classList.remove('floatAbs')
        }
        */
        ...
        if (wrap.hasClass('wrapper')) {
          //2023.08.22
          // 11.01
          //팝업 오픈 시 header ficed 클래스 사라지는 현상 제어
          if($('.dimmed').css('display') !== 'block') {
            if (scrollTop === 0 && head.hasClass('fixed')) {
              head.removeClass('fixed');
            } else if (scrollTop > 0 && !head.hasClass('fixed')) {
              head.addClass('fixed');
            }
          }
        }
  ...
  var tab = {
      ...
      tabOn: function(target, speed) {
        ...
        if (tabConData !== undefined) {
          ...
          if (tabScript.hasClass('anchorTabFixed')) {
            ...
            //2023.08.21
            //상단 fixed된 영역을 viewTab 전체 높이로 잡기
            //이전에 'fixedH = wrap.outerHeight();'으로 되어 있어 탭 버튼 선택 시 컨텐츠 스크롤 이동의 오차가 있었음
            if (wrap.closest('.viewTab').length > 0) {
              fixedH = wrap.closest('.viewTab').outerHeight();
            }
            ...
            if (con.closest('.scriptScroll').length > 0) {
              ...
              $(common.selector.bh)
                ...
                .animate(
                  {
                    // 10.11 테스트
                    // parseInt($('.stickyTab').css('top'))

                    // scrollTop: con.offset().top - fixedH + 8
                    // 23.10.31
                    scrollTop: $(common.selector.bh).scrollTop() + con.offset().top - fixedH + 8
                  },
                  500
                );
        ...
        tabScript.each(function() {
          ...
          // fewSpace
          if (!tabScript.hasClass('anchorTabFixed') && !tabScript.hasClass('fixed') && !tabScript.closest('.fewSpace').length > 1) {
    ...
    handler: {
      ...
      winScroll: function() {
        ...
        //고정 탭
        // if (viewTab.css('position') != 'sticky') {
        //   if (
        ...
        if (anchor.length > 0) {
          ...
          //2023.08.21
          //한 페이지에 탭메뉴와 셀렉트메뉴가 존재해 동일한 버튼 구조(ul > li > a)를 가져 충돌 오류 발생
          //명확히 'tabWrap' div의 자식 ul 태그를 선택
    ...
    setProperty: function(el) {
      ...
      // this.el.acc = el.parent().siblings('.contractAcc').children('.acctItem');
    },
    bind: function() {
      ...
      $('.wrapper').off('scroll.tab').on('scroll.tab', tab.handler.winScroll);
    },
    init: function(el) {
      this.setProperty($(el));
      this.bind();
    ...
    initAll: function() {
      ...
      if (this.el.tabWrap.css('overflow') === 'auto') {
        tab.methodes.tabScrollMove(this.el.tabWrap, this.el.tabWrap.find('[aria-selected=true]').closest('li'), 0);
      }
    }
  };
    ...
    init: function() {
      ...
      var options = {
        ...
        slide: function(event, ui) {
        	if(sliderId.startsWith("l_ais_slider-range")){
        		updateTabContentsText(sliderId, ui.values[0], ui.values[1], optionData.min, optionData.max);
        	}
          
          // 2023.10.26 개발 추가
          if(sliderId.startsWith("l_mes_slider_range")){
            mesSlideSetUp(sliderId, ui.values[0], ui.values[1], optionData.min, optionData.max);
          }
        }
      };
  ...
  var etc = {
    ...
    handler: {
      scroll: function() {
        ...
        //2023.08.24
        //스크롤 이동 시 'tabScrollEvt' 클래스 추가 및 제거
        if ($('.tabType05').length > 0) {
          if (currScrollTop > etc.prevScrollTop) {
            $('.tabType05').closest('.tabWrap').addClass('tabScrollEvt');
          } else {
            $('.tabType05').closest('.tabWrap').removeClass('tabScrollEvt');
          }
          etc.prevScrollTop = currScrollTop;
        }
   ...
   // 22.10.25
      ...
      $(etc.selector.listDestination).each(function() {
        ...
        var jessBtn = $this.find('>.destiWrap');
        var scrollArea = $this.find('.scrollArea');
        ...
        jessBtn.off('.etc');
        // jessBtn.off('touchstart.etc').on('touchstart.etc', function(e) {
        jessBtn.on('touchstart.etc', function(e) {  
          var sTime = e.timeStamp;
          var start = e.originalEvent.changedTouches[0].clientY;
          var moveCh = false;

          jessBtn.off('touchmove.etc').on('touchmove.etc', function(e) {
            moveCh = true;
          });

          jessBtn.off('touchend.etc').on('touchend.etc', function(e) {
            var eTime = e.timeStamp;
            var end = e.originalEvent.changedTouches[0].clientY;

            if (eTime - sTime < 700 && moveCh) {
              if (start < end && $this.hasClass('half')) {
                ...
                // if (scrollArea.scrollTop() > 0 && e.target.classList.contains('dragBtn')) {
                if (scrollArea.scrollTop() > 0 && !e.target.classList.contains('dragBtn')) {
                  return
                }
        ...
        $this.off('click.etc').on('click.etc', function(e) {
          ...
          tab.each(function(idx) {
            if ($(tab[idx]).attr('aria-selected') == 'true') {
              ...
              $(accCurrent.find('.toggleBtn')).attr('aria-expanded', 'true')
              //선택한 탭의 아코디언 영역 'on' class 추가
              accCurrent.addClass('on');
              // tab.methodes.tabScrollY(tabWrap);
            } else {
              $(acc[idx]).attr('aria-expanded', 'false')
              //선택한 탭 제외 탭들의 아코디언 영역 'on' class 제거
              $this.next().find('[data-js-tabacc="'+tab[idx].dataset.jsTabacc+'"]').removeClass('on');
            }
          })
      ...
      $(etc.selector.contractAcc).each(function() {
        ...
        $this.find('.toggleBtn').off('click.etc').on('click.etc', function(e) {
          e.preventDefault();
      ...
      */
      ...
      $(etc.selector.featureControl).each(function() {
        ...
        //매물·도구 버튼 클릭 시
        btn.off('click.tool').on('click.tool', function(){
          $(this).siblings('.option').toggleClass('show')
        })
        //매물·도구 옵션 버튼 클릭 시
        $this.find('.ico').off('click.ico').on('click.ico', function(){
      ...
      $('header .btnSwiper').each(function() {
        ...
        btn.on('click', function(e){
          ...
          if ($('.anchorTabFixed .btnSwiper').length > 0) {
            //메뉴 선택 시 가로 스크롤 이동
      ...
      $('.tabWrap > ul').each(function() {
        ...
        btn.on('click', function(e){
          ...
          // const targetEl = el.target.querySelector('ul');
          ...
          // console.log(target.offset().left ,' : ',  tab[0].scrollLeft);
          if (middle > left + tab[0].scrollLeft) {
              middle = left;
          }
        
        // gsap.to(targetEl, { scrollLeft: li.offsetLeft - middle, duration: 0.5, ease: Expo.easeOut });
          // console.log(' ::: ', target.offset().left);

          tab.stop().animate({scrollLeft: left - middle}, 400, $.easing.swing);
        })
      })
      ...
      //2023.08.17
      //textarea focus·blur시 클래스 추가
      $(etc.selector.comment).each(function() {
        var textArea = $(this).find('textarea');
        var commentInp = $(this).find('.commentInp');

        textArea.focus(function() {
          commentInp.addClass('active')
        });
      })

      //2023.08.18
      //레이어 팝업 스크롤 시 nav에 navFixed 클래스 추가 및 제거
      $(common.selector.header).each(function() {
        var head = header.el.menuWrap.find('nav');
        head.find('.menuScrollArea').scroll(function() {
          var scrollTop = $(this).scrollTop();
          var mark = $(this).find('.agencyInfo').outerHeight() + $(this).find('.mySthNum').outerHeight();

          if (scrollTop > mark && !head.hasClass('navFixed')) {
            head.addClass('navFixed');
          } else if (scrollTop < mark && head.hasClass('navFixed')) {
            head.removeClass('navFixed');
          }
        });
      })
      
      //2023.08.22
      //[매매/전세/월세/연세] 버튼 클릭 시 하단 매물리스트 영역으로 스크롤 이동  
      $(etc.selector.scrollAnchor).each(function() {
        var $this = $(this);
        
        // 10.11 추가 
        $this.off('click.etc').on('click.etc', function(){
          var fixedH = $('.scrollHeader').height();

          if ($this.closest('.areaSummary').siblings('.viewTab').length > 0) {
            fixedH += $this.closest('.areaSummary').siblings('.viewTab').outerHeight();
          }
          
          $(common.selector.bh)
          .stop()
          .animate({scrollTop: $('.detailOpen #dealList').offset().top - fixedH + 10}, 700);
        })
      })
      
      //2023.11.29
      $('.areaSelWrapResult').each(function() {
        var $this = $(this);
        var chkList = $this.find('ul.chkList');
        var layPop = $this.closest('.layPop');
        
        $this.css("max-height", layPop.height() - 258);

        chkList.on('DOMNodeInserted', function(){
          $this.css("max-height", layPop.height() - 258);
        });
        
        chkList.on('DOMNodeRemoved', function(){
          $this.css("max-height", layPop.height() - 258);
        });
      })
    },
