(function($, window) {
  ...
  handler: {
    slide: function(e, ui) {
      var options = e.data;
      var target = $(e.target);
      var amount = target.next('.slideInp').children('.amount');
      ...
      if(result.length >= 6) {
        $('.valueTip input').val(result.substring(0,1) + "," + result.substring(1,4) +  "," + result.substring(4, 6))
      //10억 미만, ex) 9억 -> 900,00
      } else if (result.length < 6) {
        $('.valueTip input').val(result.substring(0,3) +  "," + result.substring(3, 5))
      }
      //2023.07.31
  ...
  setProperty: function(el) {
    this.el.target = el;
    this.el.viewArea = el.next('.slideInp');
    this.methodes.init();
  },
  bind: function() {},
  init: function(el) {
  ...
  var etc = {
    ...
    bind: function() {
      ...
      .on('input.autoExpand', 'textarea', function() {
          ...
          if ($(this).closest('.layPop').length > 0) {
              return;
          }
          var maxRows = 10;
      ...
      //페이지별 플로팅 버튼
      $(etc.selector.pageAside).each(function() {
        var $this = $(this);
        var btn = $this.find('.btTool');
        if (btn.attr("aria-expanded") > 0){
          btn
          .off('click.pageBtn')
          .on('click.pageBtn', etc.handler.pageBtn);
        }
      ...
      $(etc.selector.contractAcc).each(function() {
        ...
        $this.off('click.etc').on('click.etc', function(e) {
          ...
          var top = $this.closest('.contractTabCont').find('.contractTab')

          // tab.methodes.tabScrollY(top);
      ...
      $(etc.selector.inpDateF).each(function() {
        ...
        input.off('input.etc').on('input.etc', function(e) {
          ...
          if(temp.length < 5) {
            ...
            input.val(temp.substring(0,4) + "-" + temp.substring(4,6) + "-" + temp.substring(6))
          } 
        });
