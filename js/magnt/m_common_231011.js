(function($, window) {
  ...
  var mySwiper = {
    ...
    setInit: function() {	// l.w.j
    	mySwiper.el.swiperWrap = el;
    	mySwiper.el.container = el.children('.swiper-container');
    	mySwiper.el.wrapper = mySwiper.el.container.children('.swiper-wrapper');
    	mySwiper.el.cont = mySwiper.el.wrapper.children('.swiper-slide');
    	mySwiper.el.prevBtn = mySwiper.el.container.siblings('.swiper-button-prev');
        mySwiper.el.nextBtn = mySwiper.el.container.siblings('.swiper-button-next');
        mySwiper.el.pagination = mySwiper.el.container.siblings('.swiper-pagination');
        mySwiper.el.paging = mySwiper.el.container.siblings('.swiper-paging');

        mySwiper.methodes.init();
     }
  };
    ...
    methodes             : {
      ...
      hide: function(target) {
        ...
        if (target.hasClass('layBtm')) {
          ...
          //외 경우 (풀팝업)
          if($('.layContent').length > 0) {
            $('.layContent').scrollTop(0);
            // $('.layContent').find('.tabWrap>ul>li>a').attr('aria-selected','false');
            // $('.layContent').find('.tabWrap>.tabType01>li>a').eq(0).attr('aria-selected','true');
            // $('.layContent').find('.tabWrap>.tabType02>li>a').eq(0).attr('aria-selected','true');
          }
          target.hide();
        }
        utils.scrollEnabled();
  ...
  var tab = {
    ...
    selector: {},
    tabWrapTop : $('.viewTab').length > 0 ? $('.viewTab').offset().top : null,
    methodes: {
      tabOn: function(target, speed) {
        ...
        tabScript.each(function() {
          ...
          // if (!tabScript.hasClass('anchorTabFixed') && !tabScript.hasClass('fixed') && !tabScript.hasClass('searchFixed')) {
          //   tab.methodes.tabScrollY($this);
          // }

          if ($this.closest('.viewTab').length > 0) {
            $this.closest('.viewTab').offset().top > 501.09375 ? tab.tabWrapTop = 501.09375 : null;
              if (tab.tabWrapTop == $this.closest('.viewTab').offset().top) {
                tab.methodes.tabScrollY($this);
              }
          }
        });
      ...
      tabScrollMove: function(wrap, target, speed) {
        ...
        var moveVal = targetLeft + scrollLeft - wrapW / 2 + target.width() / 3;
        if (moveVal < 0) {
          moveVal = 0;
        }
    ...
    handler: {
      click: function(e) {
        ...
        tab.methodes.tabOn($this.closest('li'));
      },
      scroll: function() {
        //탭 가로 스크롤 시
        var $this = $(this);
      ...
      winScroll: function() {
        ...
        //고정 탭
        // if (viewTab.length > 0) {
        //   if (viewTab.offset().top -2 <= headScrollTop && !viewTab.hasClass('add')) {
        //     viewTab.addClass('add');
        //   } else if (viewTab.offset().top > headScrollTop && viewTab.hasClass('add')) {
        //     viewTab.removeClass('add');
        //   }
        // }
  ...
  var etc = {
    ...
    bind: function() {
      ...
      //2023.08.09
      //지도 스크립트 - 2
      if (common.el.doc.find('#pop_searchSetting')) {
        $('#pop_searchSetting').each(function() {
          ...
          btn.on('click', function(e){
            ...
            .stop()
            .animate({
              scrollLeft: ((target.offset().left + target.outerWidth(true)/2) - tabWidth)
            }, 400, $.easing.swing);
        ...
        $('.wrapper .btnSwiper').each(function() {
          var $this = $(this);
          ...
          btn.on('click', function(e){
            ...
            $('.layContent .btnSwiper button').each(function(idx, el) {
            ...
            if ($('.btnSwiper').length > 0) {
              ...
              .stop()
              .animate({
                //((connectedBtn.getBoundingClientRect().left + connectedBtn.getBoundingClientRect().width/2) - tabWidth)
      ...
      $(etc.selector.scrollAnchor).each(function() {
        ...
        $this.off('click.etc').on('click.etc', function(){
          $(common.selector.bh)
          .stop()
          .animate({scrollTop: $('#dealList').offset().top - fixedH + 10}, 700);
        })
      })
    },
    init: function() {
  ...
  var mvJs = {
    ...
    ect: {
         ...
         * 더보기 유형2 갱신
         */
    	moreCont02:	etc.handler.moreContType02	// l.w.j
      /**
