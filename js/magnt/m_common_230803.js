(function($, window) {
  ...
  methodes: {
    init: function() {
      ...
      mySlider.on('slide', options, slider.handler.slide).find('.ui-slider-handle').append(valueTip);
  ...
  handler: {
    slide: function(e, ui) {
      ...
      if(result.length >= 6) {
        $('.valueTip span').val(result.substring(0,1) + "," + result.substring(1,4) +  "," + result.substring(4, 6))
      //10억 미만, ex) 9억 -> 900,00
      } else if (result.length < 6) {
        $('.valueTip span').val(result.substring(0,3) +  "," + result.substring(3, 5))
      }
      //2023.07.31
